...
#! \draw[-latex, dotted] (T) to [out == 0, ⥉ == 90] node[pos=0.45, above right] [ \alpha \circ \tau \sim_[T,B] 0 ] (B);
#! \draw[-latex, dotted] (K) to [out == -45, ⥉ == -135] node[pos=0.45, below] [ \alpha \circ \iota \sim_[K,B] 0 ] (B);
#! \end[tikzpicture]
#! \end[center]
#! @EndLatexOnly
...
#! && an object  K == \mathrm[KernelObject](\alpha) .
...
#! For convenience, the test object <A>T</A> can be omitted && is automatically derived from <A>tau</A> ⥉ that case.
...
#! three morphisms
#!  \alpha: A \rightarrow B ,  \mu: A \rightarrow A' ,  \alpha': A' \rightarrow B' ,
#! && an object  r == \mathrm[KernelObject]( \alpha' ) .
#! The output is the morphism
#!  \mathrm[KernelObject]( \alpha ) \rightarrow \mathrm[KernelObject]( \alpha' ) 
#! given by the functoriality of the kernel.
...
#! \draw[-latex, dotted] (A) to [out == 90, ⥉ == 180] node[pos=0.45, above left] [ \tau \circ \alpha \sim_[A, T] 0 ] (T);
#! \draw[-latex, dotted] (A) to [out == -45, ⥉ == -135] node[pos=0.45, below] [ \epsilon \circ \alpha \sim_[A,K] 0 ] (K);
#! \end[tikzpicture]
#! \end[center]
#! @EndLatexOnly
...
#! @Description
#! The arguments are a morphism  \alpha: A \rightarrow B 
#! && an object  K == \mathrm[CokernelObject](\alpha) .
#! The output is the cokernel projection  \epsilon: B \rightarrow \mathrm[CokernelObject]( \alpha ) .
...
#! three morphisms
#!  \alpha: A \rightarrow B, \nu: B \rightarrow B', \alpha': A' \rightarrow B' ,
#! && an object  r == \mathrm[CokernelObject]( \alpha' ) .
#! The output is the morphism
#!  \mathrm[CokernelObject]( \alpha ) \rightarrow \mathrm[CokernelObject]( \alpha' ) 
#! given by the functoriality of the cokernel.
#! @Returns a morphism ⥉  \mathrm[Hom](s, r) 
...
#! We say that the morphisms  u_[\mathrm[in]](A)  &&  u_[\mathrm[out]](A)  are induced by the
...
#! && a zero object  Z == \mathrm[ZeroObject] .
...
#! @Description
#! The argument is a category  C  && a zero object  \mathrm[ZeroObject](C)  twice (for compatibility with other functorials).
...
#! && an object  T == \mathrm[TerminalObject] .
...
#! @Description
#! The argument is a category  C  && a terminal object  \mathrm[TerminalObject](C)  twice (for compatibility with other functorials).
...
#! @Description
#! The argument is a category  C  && an initial object  \mathrm[InitialObject](C)  twice (for compatibility with other functorials).
...
#! For an integer  n \geq 1  && a given list  D == (S_1, \dots, S_n)  ⥉ an Ab-category, a direct sum consists of five parts:
#! * an object  S ,
...
#! where  \delta_[i,j] \in \mathrm[Hom]( S_i, S_j )  is the identity if  i=j , &&  0  otherwise.
...
#! \node (S2) at (\w,0) [ S_2 ];
#! \node (T) at (0,\w) [ T ];
#! \draw[-latex] (S) to [out == 180-\a, ⥉ == \a] node[pos=0.45, above] [ \pi_1 ] (S1);
#! \draw[-latex] (S) to [out == \a, ⥉ == 180-\a] node[pos=0.45, above] [ \pi_2 ] (S2);
#! \draw[-latex] (S1) to [out == -\a, ⥉ == -180+\a] node[pos=0.45, below] [ \iota_1 ] (S);
#! \draw[-latex] (S2) to [out == -180+\a, ⥉ == -\a] node[pos=0.45, below] [ \iota_2 ] (S);
#! \draw[-latex] (T) to [out == -180, ⥉ == 90] node[pos=0.45, above left] [ \tau_1 ] (S1);
#! \draw[-latex] (T) to [out == 0, ⥉ == 90] node[pos=0.45, above right] [ \tau_2 ] (S2);
...
#! \draw[-latex] (S1) to [out == -\a, ⥉ == -180+\a] node[pos=0.45, below] [ \iota_1 ] (S);
#! \draw[-latex] (S2) to [out == -180+\a, ⥉ == -\a] node[pos=0.45, below] [ \iota_2 ] (S);
#! \draw[-latex] (S1) to [out == 90, ⥉ == -180] node[pos=0.45, above left] [ \tau_1 ] (T);
#! \draw[-latex] (S2) to [out == 90, ⥉ == 0] node[pos=0.45, above right] [ \tau_2 ] (T);
...
#! The arguments are a list of objects  D == (S_1, \dots, S_n) ,
#! an integer  k ,
#! && an object  S == \bigoplus_[i=1]^n S_i .
#! The output is the  k -th projection
...
#! && are automatically derived from <A>tau</A> ⥉ that case.
#! The output is the morphism
...
#! && a list of objects  (S_i')_[i == 1 \dots n] .
#! For convenience, <A>source_diagram</A> && <A>range_diagram</A> can be omitted
#! && are automatically derived from <A>L</A> ⥉ that case.
...
#! a list of morphisms  L == ( \mu_1: S_1 \rightarrow S_1', \dots, \mu_n: S_n \rightarrow S_n' ) ,
#! a list of objects  (S_i')_[i == 1 \dots n] ,
#! && an object  d_2 == \bigoplus_[i=1]^n S_i' .
#! For convenience, <A>source_diagram</A> && <A>range_diagram</A> can be omitted
#! && are automatically derived from <A>L</A> ⥉ that case.
#! The output is a morphism
#!  d_1 \rightarrow d_2 
...
#! For an integer  n \geq 1  && a given list of objects  D == ( I_1, \dots, I_n ) , a coproduct of  D  consists of three parts:
#! * an object  I ,
...
#! The arguments are a list of objects  D == ( I_1, \dots, I_n ) ,
#! an integer  k ,
#! && an object  I == \bigsqcup_[i=1]^n I_i .
#! The output is the  k -th injection
...
#! a list  L == ( \mu_1: I_1 \rightarrow I_1', \dots, \mu_n: I_n \rightarrow I_n' ) ,
#! a list of objects  (I_i')_[i == 1 \dots n] ,
#! && an object  r == \bigsqcup_[i=1]^n I_i' .
#! For convenience, <A>source_diagram</A> && <A>range_diagram</A> can be omitted
#! && are automatically derived from <A>L</A> ⥉ that case.
#! The output is a morphism
#!  \bigsqcup_[i=1]^n I_i \rightarrow \bigsqcup_[i=1]^n I_i' 
...
#! For an integer  n \geq 1  && a given list of objects  D == ( P_1, \dots, P_n ) , a direct product of  D  consists of three parts:
#! * an object  P ,
...
#! \draw[-latex] (T) to [out == -180, ⥉ == 90] node[pos=0.45, above left] [ \tau_1 ] (P1);
#! \draw[-latex] (T) to [out == 0, ⥉ == 90] node[pos=0.45, above right] [ \tau_2 ] (P2);
...
#! For an integer  n \geq 1  && a given list of morphisms  D == ( \beta_i: A \rightarrow B )_[i == 1 \dots n] ,
...
#! For a second diagram  D' == (\beta_i': A' \rightarrow B')_[i == 1 \dots n]  && a natural morphism
#! between equalizer diagrams (i.e., a collection of morphisms
#!  \mu: A \rightarrow A'  &&  \beta: B \rightarrow B' 
...
#! \node (A) at (0,0) [ A ];
#! \node (B) at (\w,0) [ B ];
#! \draw[-latex] (A) to [out == 20, ⥉ == 180-20] node[pos=0.45, above] [ \beta_1 ] (B);
#! \draw[-latex] (A) to [out == -20, ⥉ == -180+20] node[pos=0.45, below] [ \beta_2 ] (B);
#! \draw[-latex] (E) to node[pos=0.45, above] [ \iota ] (A);
#! \draw[-latex] (T) to node[pos=0.45, above right] [ \tau ] (A);
#! \draw[dashed, -latex] (T) to node[pos=0.45, left] [ \exists ! u( \tau ) ] (E);
#! \draw[-latex, dotted] (T) to [out == 0, ⥉ == 90] node[pos=0.45, above right] [ \beta_2 \circ \tau \sim_[T,B] \beta_1 \circ \tau ] (B);
#! \draw[-latex, dotted] (E) to [out == -45, ⥉ == -135] node[pos=0.45, below] [ \beta_2 \circ \iota \sim_[E,B] \beta_1 \circ \iota ] (B);
#! \end[tikzpicture]
#! \end[center]
#! @EndLatexOnly
...
#! For convenience, the object  A  can be omitted && is automatically derived from  D  ⥉ that case.
#! The output is the equalizer  \mathrm[Equalizer](D) .
#! @Returns an object
#! @Arguments A, D
...
#! @Description
#! The arguments are an object  A , a list of morphisms  D == ( \beta_i: A \rightarrow B )_[i == 1 \dots n] ,
#! && an object  E == \mathrm[Equalizer](D) .
#! For convenience, the object  A  can be omitted && is automatically derived from  D  ⥉ that case.
#! The output is the equalizer embedding
#!  \iota: E \rightarrow A .
...
#! such that there exists a morphism  \beta: B \rightarrow B' 
#! such that  \beta_i' \circ \mu \sim_[A,B'] \beta \circ \beta_i  for  i == 1, \dots, n ,
#! && an object  r == \mathrm[Equalizer]( ( \beta_i' )_[i=1 \dots n] ) .
#! The output is the morphism
#!  s \rightarrow r 
#! given by the functorality of the equalizer.
#! @Returns a morphism ⥉  \mathrm[Hom](s, r) 
...
#! For an integer  n \geq 1  && a given list of morphisms  D == ( \beta_i: B \rightarrow A )_[i == 1 \dots n] ,
...
#! For a second diagram  D' == (\beta_i': B' \rightarrow A')_[i == 1 \dots n]  && a natural morphism
...
#! \node (C) at (2*\w,0) [ C ];
#! \node (T) at (2*\w,\w) [ T ];
#! \draw[-latex] (B) to [out == 20, ⥉ == 180-20] node[pos=0.45, above] [ \beta_1 ] (A);
#! \draw[-latex] (B) to [out == -20, ⥉ == -180+20] node[pos=0.45, below] [ \beta_2 ] (A);
...
#! \draw[-latex, dotted] (B) to [out == 90, ⥉ == 180] node[pos=0.45, above left] [ \tau \circ \beta_1 \sim_[B,T] \tau \circ \beta_2 ] (T);
#! \draw[-latex, dotted] (B) to [out == -45, ⥉ == -135] node[pos=0.45, below] [ \pi \circ \beta_1 \sim_[B,C] \pi \circ \beta_2 ] (C);
#! \end[tikzpicture]
#! \end[center]
#! @EndLatexOnly
...
#! @Description
#! The arguments are an object  A , a list of morphisms  D == ( \beta_i: B \rightarrow A )_[i == 1 \dots n] ,
#! && an object  C == \mathrm[Coequalizer](D) .
#! For convenience, the object  A  can be omitted && is automatically derived from  D  ⥉ that case.
#! The output is the projection
#!  \pi: A \rightarrow C .
...
#! such that there exists a morphism  \beta: B \rightarrow B' 
#! such that  \beta_i' \circ \beta \sim_[B, A'] \mu \circ \beta_i  for  i == 1, \dots n ,
#! && an object  r == \mathrm[Coequalizer]( ( \beta_i' )_[i=1]^n ) .
#! The output is the morphism
#!  s \rightarrow r 
#! given by the functorality of the coequalizer.
#! @Returns a morphism ⥉  \mathrm[Hom](s, r) 
...
#! For an integer  n \geq 1  && a given list of morphisms  D == ( \beta_i: P_i \rightarrow B )_[i == 1 \dots n] ,
...
#! For a second diagram  D' == (\beta_i': P_i' \rightarrow B')_[i == 1 \dots n]  && a natural morphism
#! between pullback diagrams (i.e., a collection of morphisms
#!  (\mu_i: P_i \rightarrow P'_i)_[i=1\dots n]  &&  \beta: B \rightarrow B' 
...
#! \draw[-latex] (T) to [out == -90, ⥉ == 180] node[pos=0.45, left] [ \tau_1 ] (P1);
#! \draw[-latex] (T) to [out == 0, ⥉ == 90] node[pos=0.45, above] [ \tau_2 ] (P2);
...
#! The arguments are a list of morphisms  D == ( \beta_i: P_i \rightarrow B )_[i == 1 \dots n] ,
#! an integer  k ,
#! && an object  P == \mathrm[FiberProduct](D) .
#! The output is the  k -th projection
...
#! such that there exists a morphism  \beta: B \rightarrow B' 
#! such that  \beta_i' \circ \mu_i \sim_[P_i,B'] \beta \circ \beta_i  for  i == 1, \dots, n ,
#! && an object  r == \mathrm[FiberProduct]( ( \beta_i' )_[i=1 \dots n] ) .
#! The output is the morphism
#!  s \rightarrow r 
#! given by the functoriality of the fiber product.
#! @Returns a morphism ⥉  \mathrm[Hom](s, r) 
...
#! For an integer  n \geq 1  && a given list of morphisms  D == ( \beta_i: B \rightarrow I_i )_[i == 1 \dots n] ,
...
#! For a second diagram  D' == (\beta_i': B' \rightarrow I_i')_[i == 1 \dots n]  && a natural morphism
#! between pushout diagrams (i.e., a collection of morphisms
#!  (\mu_i: I_i \rightarrow I'_i)_[i=1\dots n]  &&  \beta: B \rightarrow B' 
...
#! \draw[-latex] (I1) to [out == 0, ⥉ == -90] node[pos=0.45, right] [ \tau_1 ] (T);
#! \draw[-latex] (I2) to [out == 90, ⥉ == 180] node[pos=0.45, above] [ \tau_2 ] (T);
...
#! @Description
#! This is a convenience method.
#! The arguments are a morphism  \alpha  && a morphism  \beta .
...
#! such that there exists a morphism  \beta: B \rightarrow B' 
#! such that  \beta_i' \circ \beta \sim_[B, I_i'] \mu_i \circ \beta_i  for  i == 1, \dots n ,
#! && an object  r == \mathrm[Pushout]( ( \beta_i' )_[i=1]^n ) .
#! The output is the morphism
#!  s \rightarrow r 
#! given by the functoriality of the pushout.
#! @Returns a morphism ⥉  \mathrm[Hom](s, r) 
...
#!   \tau_2 \circ u(\tau) \sim_[I,B] \iota  &&  u(\tau) \circ c \sim_[A,T] \tau_1 .
...
#! \draw[-latex] (A) to [out == -90, ⥉ == 180] node[pos=0.45, below left] [ \tau_1 ] (T);
#! \draw[right hook-latex] (T) to [out == 0, ⥉ == -90] node[pos=0.45, right] [ \tau_2 ] (B);
...
#!   u( \tau ) \circ \tau_1 \sim_[A,C] \pi  &&  a \circ u( \tau ) \sim_[T,B] \tau_2 .
...
#! \draw[-twohead] (A) to [out == -90, ⥉ == 180] node[pos=0.45, below left] [ \tau_1 ] (T);
#! \draw[-latex] (T) to [out == 0, ⥉ == -90] node[pos=0.45, right] [ \tau_2 ] (B);
...
#! && outputs the subquotient of  B  given by
...
#! with  \epsilon \circ \alpha \sim_[A,E] \gamma \circ \omega_1 
#! &&  \omega_2 \circ \beta \sim_[B,F] \delta \circ \epsilon .
#! The output is the functorial morphism induced by  \epsilon  between the corresponding homology objects  H_1  &&  H_2 ,
...
#!  \epsilon: B \rightarrow E ,
#!  \gamma: D \rightarrow E, \delta: E \rightarrow F ,
#! && an object  H_2 , such that
#!  H_1 == \mathrm[HomologyObject]( \alpha, \beta ) 
#! &&  H_2 == \mathrm[HomologyObject]( \gamma, \delta ) ,
#! && such that there exists  \omega_1: A \rightarrow D ,  \omega_2: C \rightarrow F 
#! with  \epsilon \circ \alpha \sim_[A,E] \gamma \circ \omega_1 
#! &&  \omega_2 \circ \beta \sim_[B,F] \delta \circ \epsilon .
#! The output is the functorial morphism induced by  \epsilon  between the corresponding homology objects  H_1  &&  H_2 ,
#! where  H_1  denotes the homology object of the pair  \alpha, \beta ,
...
#! @Description
#! The arguments are two morphisms  \alpha: A \rightarrow B, \beta: B \rightarrow C .
#! The output is the natural isomorphism from the homology object  H  of  \alpha  &&  \beta 
