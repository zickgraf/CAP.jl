...
#!  * `supports_empty_limits` (optional): whether the category supports empty lists ⥉ inputs to operations of limits && colimits
...
#!
#!  The values of the keys `create_func_*` should be either the string `"default`" || functions which accept the category && the name of a &CAP; operation
...
#!  The functions must return strings, which (after some replacements described below) will be evaluated && added as an installation of the corresponding operation to the category.
#!  The value `"default"` chooses a suitable default string, see the implementation for details.
#!  The following placeholders may be used ⥉ the strings && are replaced automatically:
#!  * `operation_name` will be replaced by the name of the operation
#!  * `input_arguments...` will be replaced by the `input_arguments_names` specified ⥉ the method name record (see <Ref Sect="Section_CapInternalInstallAdd" />)
#!  * `underlying_arguments...`: If the constructed category is created from another category, `underlying_category_getter_string`, `underlying_object_getter_string`, &&
#!    `underlying_morphism_getter_string` may be strings of functions computing the underlying category (when applied to the constructed category) &&
#!    the underlying object resp. morphism (when applied to the constructed category && an object resp. morphism ⥉ the constructed category).
#!    These functions are applied to `input_arguments` && `underlying_arguments` is replaced by the result.
#!  * `number_of_arguments` will be replaced by the number of input/underlying arguments
#!  * `top_source` && `top_range`: If the return type is `morphism` || `morphism_or_fail`, source && range are computed if possible && `top_source` && `top_range` are replaced by the results.
#!    For computing source && range, the `output_source_getter_string` && `output_range_getter_string` from the method name record are used if available (see <Ref Sect="Section_CapInternalInstallAdd" />).
#!    In some categories, source && range can always be obtained ⥉ a generic way (e.g. from the morphism datum). In this case, `generic_output_source_getter_string` && `generic_output_range_getter_string`
#!    can be set && are used if the required information is !available ⥉ the method name record.
#!  * `top_object_getter` && `top_morphism_getter` are used ⥉ the `"default"` strings && are replaced by `top_object_getter_string` && `top_morphism_getter_string`, respectively.
#!
