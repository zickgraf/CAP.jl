...
#! * ObjectDatum
#! * MorphismDatum
#! ⥉ order to reflect the desired interpretation with
...
#! to the top category <C>cat_n</C> ⥉ the tower.
...
#!  To avoid inconsistencies, either all || none of those options should be set. If they are set, also the following options should be set:
#!  * `modeling_tower_object_constructor`: a function which gets the wrapper category && an object datum (in the sense of `object_datum`) && returns the corresponding modeling object ⥉ the modeling category,
#!  * `modeling_tower_object_datum`: a function which gets the wrapper category && an object ⥉ the modeling category && returns the corresponding object datum (in the sense of `object_datum`),
#!  * `modeling_tower_morphism_constructor`: a function which gets the wrapper category, a source ⥉ the modeling category, a morphism datum (in the sense of `morphism_datum`), && a range ⥉ the modeling category && returns the corresponding modeling morphism ⥉ the modeling category,
...
#! @Description
#!  Returns the object modeling the object <A>obj</A> ⥉ <A>cat</A>.
...
#! @Description
#!  Returns the object modeled by the object <A>obj</A> ⥉ the modeling category of <A>cat</A>.
#!  <A>cat</A> must be a CAP category which has been created as a wrapper CAP category (but !necessarily uses the default data structure).
#! @Arguments cat, obj
#! @Returns a CAP category object
...
#! @Description
#!  Returns the morphism modeling the morphism <A>mor</A> ⥉ <A>cat</A>.
#!  <A>cat</A> must be a CAP category which has been created as a wrapper CAP category (but !necessarily uses the default data structure).
#! @Arguments cat, mor
