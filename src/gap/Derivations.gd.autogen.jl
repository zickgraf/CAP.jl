...
#!  implements, && which other operations it relies on.
...
#!  identify this derivation, && is useful only for debugging
#!  purposes.
...
#!  first entry the name of an operation, as second entry an integer && as
#!  third entry optionally a function. This function should accept the
#!  category && return a category for which the operation in the first
...
#!  together with their multiplicities && category getters.
...
#! @Description
#!  A derivation graph consists of a set of operations && a set of derivations
#!  specifying how some operations can be implemented ⥉ terms of other operations.
@DeclareFilter( "IsDerivedMethodGraph", IsAttributeStoringRep );

#! @Description
#!  Make a derivation graph containing the given set of operations && no derivations.
...
#!  This is used ⥉ extensions of CAP which want to have their own basic operations,
...
####################################
##
#! @Section Managing Derivations ⥉ a Category
##
####################################
...
#!  This should only be done once for every category, && the category should
...
#!  This causes all operations that can be derived, directly || indirectly,
...
#! @Description
#!  A string min heap is a min heap where every node contains a string label && an
#!  integer key.
...
#!  && updates the heap's internal mapping of labels to list indices.
