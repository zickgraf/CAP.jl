...
#!  this function wraps the function with a wrapper function to fit ⥉ the CAP context.
#!  For the mentioned binary direct sum one can call this function with
#!  <C>"BinaryDirectSumToDirectSum"</C> as <A>prepare_function</A>, the category, &&
...
#!  The first argument is the prepare function itself. It should always be a function that takes a category &&
#!  a function && returns a function.
#!  The argument <A>name</A> is the name of the prepare function, which is used ⥉ <C>CAPOperationPrepareFunction</C>.
